...
fn scoped(client: &NominalClient, scope: &ScopeArgs) -> (ExtractorsClient, ContainerImagesClient) {
...
pub async fn handle(cmd: ExtractorCommands, client: NominalClient) -> anyhow::Result<()> {
    match cmd {
        ...
        ExtractorCommands::Image { command } => images(command, client).await,
    }
}
async fn images(cmd: ImageCommands, client: NominalClient) -> anyhow::Result<()> {
    match cmd {
