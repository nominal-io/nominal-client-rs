        ...
        match (&self.timestamp_column, &self.timestamp_type) {
            (Some(column), Some(spec)) => {
                ...
                let unit = match spec.as_str() {
                    ...
                    "s" | "secs" | "seconds" => TimeUnit::Seconds,
                    "minutes" => TimeUnit::Minutes,
                    "hours" => TimeUnit::Hours,
                    "days" => TimeUnit::Days,
                    _ => anyhow::bail!("unknown timestamp type: {spec}"),
                };
...
pub async fn handle(a: ContainerizedArgs, client: NominalClient) -> anyhow::Result<()> {
