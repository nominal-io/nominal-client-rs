...
// PKCS#11 returns raw r || s; TLS requires DER SEQUENCE { INTEGER r, INTEGER s } (RFC 3279 §2.2.3).

/// Convert a raw PKCS#11 ECDSA signature (`r || s`) to DER. Supports P-256 and P-384.
...
/// Encode `n` as a DER INTEGER. Strips leading zeros; prepends `0x00` if MSB is set.
...
mod tests {
    ...
    /// Read a DER length field. Returns (value, bytes consumed).
    ...
    /// Parse a DER INTEGER. Returns value bytes and total bytes consumed.
    ...
    fn known_p256_byte_sequence() {
        // Expected encoding: 30 44 02 20 01..20 02 20 21..40
