...
mod pagination_tests {
    ...
    async fn job_search_returns_all_pages_without_extra_job_requests() {
        ...
        let server = HttpFixture::new(move |request| {
            ...
            assert_eq!(request["filter"]["and"][0]["statuses"], serde_json::json!(["COMPLETED"]));
            assert_eq!(request["filter"]["and"].as_array().unwrap().len(), 1);
            assert_eq!(request.get("nextPageToken"), (page == 1).then_some(&serde_json::json!("next")));
            async move {
                let mut response = serde_json::json!({"ingestJobs":[ingest_job(&format!("ri.ingest.main.job.page{page}"))]});
                if page == 0 { response["nextPageToken"] = "next".into(); }
                response
            }
        }).await;
