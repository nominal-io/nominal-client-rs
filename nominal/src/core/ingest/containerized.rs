...
mod request_tests {
    ...
    async fn containerized_submission_returns_the_job_id_without_fetching_metadata() {
        ...
        let server = HttpFixture::new(move |request| {
            ...
            assert_eq!(request.uri().path(), "/api/ingest/v1/ingest");
            async { serde_json::json!({"ingestJobRid":"ri.ingest.main.job.test",
        ...
        let result = server
            .run(ingest.submit_containerized(ContainerizedOpts::new(
