...
fn source(upload: &PendingUpload, locations: &BTreeMap<usize, String>) -> Option<p::IngestSource> {
    Some(p::IngestSource {
        source: Some(p::ingest_source::Source::S3(p::S3IngestSource {
            path: locations.get(&upload.id)?.clone(),
        })),
    })
}
pub(super) fn encode(
    item: &PendingItem,
    locations: &BTreeMap<usize, String>,
) -> Option<p::IngestItem> {
    ...
    let (item, tags) = match item {
        ...
        PendingItem::Containerized { sources, options } => (
            Item::Containerized(p::ContainerizedIngestItem {
                ...
                sources: sources
                    .iter()
                    .map(|s| Some((s.name.clone(), source(s, locations)?)))
                    .collect::<Option<_>>()?,
