use crate::{Error, JobTimestampMetadata, Result};
...
pub(crate) struct Environment {
    ...
    pub timestamp: Option<JobTimestampMetadata>,
}
fn decode<T: DeserializeOwned>(env: &BTreeMap<String, String>, key: &str) -> Result<Option<T>> {
    env.get(key)
        .filter(|s| !s.is_empty())
        .map(|s| {
            serde_json::from_str::<Option<T>>(s).map_err(|source| Error::Metadata {
                variable: key.into(),
                source,
            })
        })
...
impl Environment {
    pub fn new(env: BTreeMap<String, String>, manifest: bool) -> Result<Self> {
        if let Some(format) = env.get("_NOMINAL_OUTPUT_FORMAT").filter(|s| !s.is_empty()) {
            if (format == "MANIFEST") != manifest {
